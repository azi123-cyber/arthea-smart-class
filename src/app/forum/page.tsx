            ...
            {topics.find(t => t.id === selectedTopic) && (() => {
              ...
              return (
                <>
                  ...
                  {/* Reply Input Bar */}
                  <div className="fixed md:sticky bottom-[3.5rem] md:bottom-6 left-2 right-2 md:left-auto md:right-auto md:mt-12 md:mx-6 glass rounded-[2rem] p-2 md:p-3 z-[55] shadow-[0_-10px_40px_rgba(0,0,0,0.08)] dark:shadow-[0_-10px_40px_rgba(0,0,0,0.5)] border-2 border-white/40 dark:border-white/10 bg-white/95 dark:bg-[#0b141a]/95 backdrop-blur-xl mb-0 transition-all">
                    ...
                    <div className="flex gap-3 items-center">
                      ...
                      <button onClick={() => handleReply(selectedTopic)} disabled={uploadingFile || (!replyText && !replyFile)}
                        className="bg-primary hover:bg-primary-dark text-white flex items-center justify-center flex-shrink-0 transition-all shadow-xl shadow-primary/20 h-10 md:h-12 rounded-2xl md:w-auto px-5 md:!px-8 md:!py-4 gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
                        {uploadingFile ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <Send size={18} />}
                        <span className="inline font-bold text-sm md:text-base">Kirim</span>
                      </button>
                    </div>
                  </div>
